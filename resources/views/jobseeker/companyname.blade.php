   ...
   @if(isset($company) && !isset($company->message))
    @forelse($company as $companies)
        ...
        <div class="well">
          ...
          <a href="{{url('jobseeker/unblock/'.$companies->employer_id)}}" class="btn btn-success">Unblock</a>
          
          @else
          <a href="{{url('jobseeker/block/'.$companies->employer_id)}}" class="btn btn-warning">Block</a>
         @endif
        </div>
