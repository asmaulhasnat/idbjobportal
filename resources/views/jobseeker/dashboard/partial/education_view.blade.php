<div class="row">
    ...
    <div class="well well-sm">
		...
		@foreach($academicsdata as $acadata)
				  ...
				  <div class="well" data-toggle="collapse" data-target="#demo{{$i++}}"><h2 class="btn btn-block btn-default icon">{{isset($levelNames) && count($levelNames[$acadata->lavelofeducation_id])>0?$levelNames[$acadata->lavelofeducation_id][0]:'level'}}<i id='#imran' class="fa fa-angle-double-down pull-right" style="font-size:28px;"></i></h2></div>
				    ...
				    <table class="table table-bordered" id="carrer_table">
								<tbody>
									...
									<tr>
										...
										<td>{{isset($levelNames) && count($levelNames[$acadata->lavelofeducation_id])>0?$levelNames[$acadata->lavelofeducation_id][0]:null}}</td>
									</tr>
									<tr>
										<th>Exam Title:</th>
										<td>{{isset($examtitlename) && count($examtitlename)>0?$examtitlename[$acadata->exam_title_id][0]:null}}</td>
									</tr>
									<tr>
										<th>Group/Major :</th>
										<td>{{isset($majorename[$acadata->groupormajor_id]) && count($majorename[$acadata->groupormajor_id])>0?$majorename[$acadata->groupormajor_id][0]:null}}</td>
									</tr>
									<tr>
										<th>Result :</th>
