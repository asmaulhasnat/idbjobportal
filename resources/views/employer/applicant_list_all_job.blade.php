		...
		<div class="tab-content">
			  <div id="alapplicant" class="tab-pane fade in active">
					...
					@if($retraive_aproved_jobs_info->id==$retrive_total_applied_info->job_id)
				  		<tr>
				  			<td><a href="{{url('employer/cv/'.$retrive_total_applied_info->jobseeker_id.'/'.$retrive_total_applied_info->job_id)}}">{{$retrive_total_applied_info->full_name}}
				  			</a>
				  			</td>
							...
							<td>
								<form action="{{url('employer/shorted_list')}}" method="post" id="short_list_form">
								{{csrf_field()}}
			    ...
			    <table class="table short_applicant">
					...
					<tr>
						...
						@foreach($retrive_total_shortlisted_applicants as $retrive_total_applied_info)
							...
							<tr>
								<td><a href="{{url('employer/cv/'.$retrive_total_applied_info->jobseeker_id.'/'.$retrive_total_applied_info->job_id)}}">{{$retrive_total_applied_info->full_name}}</a></td>
								
								<td>{{$retrive_total_applied_info->jobseeker_phone1}}</td>
								<td>{{$retrive_total_applied_info->jobseeker_email}}</td>
								
								<td>
									<form action="{{url('employer/shorted_list')}}" method="post" id="short_list_remove_form">
									{{csrf_field()}}
									<input type="hidden" value="{{ $retrive_total_applied_info->jobseeker_id }}"   name="jobseeker_id" id="jobseeker_id">
